...
public class KleiPermitDioramaVis_ArtablePainting : KMonoBehaviour, IKleiPermitDioramaVisTarget
{
  ...
  public void ConfigureWith(PermitResource permit, PermitPresentationInfo permitPresInfo)
  {
    ...
    rectTransform.anchoredPosition = Vector2.op_Addition(rectTransform.anchoredPosition, new Vector2(0.0f, (float) (-176.0 * (double) buildingDef.HeightInCells / 2.0 + 176.0)));
    ((Transform) Util.rectTransform((Component) this.buildingKAnim)).localScale = Vector3.op_Multiply(Vector3.one, 0.9f);
  }
}
