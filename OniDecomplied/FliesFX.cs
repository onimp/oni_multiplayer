...
public class FliesFX : GameStateMachine<FliesFX, FliesFX.Instance>
{
    ...
    GameStateMachine<FliesFX, FliesFX.Instance, IStateMachineTarget, object>.GameInstance
  {
    public Instance(IStateMachineTarget master, Vector3 offset)
      : base(master)
    {
      this.sm.fx.Set(((Component) FXHelpers.CreateEffect("fly_swarm_kanim", Vector3.op_Addition(TransformExtensions.GetPosition(this.smi.master.transform), offset), this.smi.master.transform)).gameObject, this.smi);
    }
