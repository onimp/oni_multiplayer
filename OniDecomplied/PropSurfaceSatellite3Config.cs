...
public class PropSurfaceSatellite3Config : IEntityConfig
{
  ...
  public GameObject CreatePrefab()
  {
    ...
    KAnimFile anim = Assets.GetAnim(HashedString.op_Implicit("satellite3_kanim"));
    EffectorValues decor = tieR0_1;
    EffectorValues noise = tieR0_2;
    List<Tag> additionalTags = new List<Tag>();
    additionalTags.Add(GameTags.Gravitas);
    GameObject placedEntity = EntityTemplates.CreatePlacedEntity(id, name, desc, 50f, anim, "off", Grid.SceneLayer.Building, 6, 6, decor, noise, additionalTags: additionalTags);
  ...
  {
    KMonoBehaviourExtensions.Subscribe(inst, -372600542, (Action<object>) (locker => this.OnLockerLooted(inst)));
  ...
  {
    GameObject gameObject = Util.KInstantiate(Assets.GetPrefab(Tag.op_Implicit(ArtifactSelector.Instance.GetUniqueArtifactID())), inst.transform.position);
